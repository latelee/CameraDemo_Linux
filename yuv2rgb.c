...
void yuv422packed_to_yuv422sp(YUV_TYPE type1, YUV_TYPE type2, unsigned char* yuv, unsigned char* yuv422sp, int width, int height)
{
	int i = 0;
	unsigned char* p_y;
	unsigned char* p_uv;
...
// 由于只是在打包格式之间进行转换，所以直接使用指针索引的方式，根据前后两种Y、U、V分量的偏移统一进行转换
void yuv422packed_to_422packed(YUV_TYPE type1, YUV_TYPE type2, unsigned char* yuv422p, unsigned char* yuv, int width, int height)
{
    ...
    {
		p_yuvout[out_idxy1] = p_yuv[idxy1];
		p_yuvout[out_idxu] = p_yuv[idxu];
		p_yuvout[out_idxy2] = p_yuv[idxy2];
		p_yuvout[out_idxv] = p_yuv[idxv];
...
// 隔行提取
void yuv422packed_to_yuv420p(YUV_TYPE type1, YUV_TYPE type2, unsigned char* yuv, unsigned char* yuv420p, int width, int height)
{
    int i = 0;
    int j = 0;
    int k = 0;
	...
	for (i = 0; i < height; i += 2) // 隔行提取，从而实现从422到420转换
	{
		...
		{
			p_u[k] = p_yuv[j + i*width * 2 + idxu];
			p_v[k] = p_yuv[j + i*width * 2 + idxv];
			k++;
		}
	}
}

void yuv422packed_to_yuv420sp(YUV_TYPE type1, YUV_TYPE type2, unsigned char* yuv, unsigned char* yuv420sp, int width, int height)
{
	int i = 0;
	int j = 0;
	int k = 0;
